#!/usr/bin/env python
#Version 05_dbs, 2017-07-20, by Daniel B. Stribling
#  Modified some variable names, added check to ensure that (on HPC) the 
#  Current loaded version of BWA is 0.7.7. This is based on failed testing
#  With BWA/0.7.15, but there may be supported versions between these 
#  Versions.
...
def SplitSAMPE (fname,odir,summname):
	...
	#Setting flags
	flags_oppositestrand = ["81","97","145","161"]
	...
	for line in SAM:
		...
		elif line[1] in flags_oppositestrand:
			print >> OPPOSITE,'\t'.join(line)
	...
	#Print summary
	count_names = ["name","total_reads","counter_opposite_strand_read","counter_grayones","counter_unmapped_read1","counter_unmapped_read2","counter_both_unmapped_read1","counter_both_unmapped_read2","counter_mapped_read1","counter_mapped_read2","counter_ambiguous_read1","counter_ambiguous_read2"] 
	count_values = [summname,total,counter_oppositestrand,counter_grayones,counter_unmappedread1,counter_unmappedread2,counter_bothunmapped1,counter_bothunmapped2,counter_mapped1,counter_mapped2,counter_ambiguous1,counter_ambiguous2]
...
def SplitSAMSE (fname,odir,summname):
	...
	flags_unmappedreads = ["4"]
        flags_secondary = ["256"]
        flags_secondary_oppositestrand = ["272"]
	...
	MAPPED = open(os.path.join(odir,name+'_mapped.sam'),'w')
	AMBIGUOUS = open(os.path.join(odir,name+'_ambiguous.sam'),'w')
        SECONDARY = open(os.path.join(odir,name+'_secondary.sam'),'w')
        SECONDARY_OPPOSITE = open(os.path.join(odir,name+'_secondary_opposite.sam'),'w')
	...
	for line in SAM:
		...
		#Getting & printing "OPPOSITE" reads
			...
			counter_oppositestrand += 1
			total += 1
                #Getting & printing "SECONDARY" reads
                elif line[1] in flags_secondary:
                        print >> SECONDARY,'\t'.join(line)
                        counter_secondary += 1
                        total += 1
                #Getting & printing "SECONDARY_OPPOSITE" reads
                elif line[1] in flags_secondary_oppositestrand:
                        print >> SECONDARY_OPPOSITE,'\t'.join(line)
                        counter_secondary_opposite += 1
                        total += 1
		#Getting & printing AMBIGUOUS reads, those who are not ambiguous are store as mapped reads
	...
	#Print summary
	count_names = ["name","total_reads","count_mapped_read_opposite_strand","count_unmapped_read","count_mapped_read","count_ambiguous_read", "count_secondary_read","count_secondary_read_opposite_strand"] 
	count_values = [summname,total,counter_oppositestrand,counter_unmappedread,counter_mapped,
                        counter_ambiguous, counter_secondary,counter_secondary_opposite]
	count_values = map(str,count_values)
	print >> SUMMARY,','.join(count_names)
	print >> SUMMARY,','.join(count_values)
