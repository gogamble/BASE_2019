...
#####turns existing code into functions with separate functions for simulated vs real data
#####adds additional argument to indicate whether dataset is a real or simulated dataset
...
def getOptions():
    ...
    parser.add_argument('-simdesign','--simdesign',dest='simdesign', action='store', required=True, help='Design file containing split simulated fastq fqNames and genotype information [Required]')
    ...
    parser.add_argument('-simpath','--simpath',dest='simpath', action='store', required=True, help='Path to directory containing simulated ase_count_tables [Required]')
    ...
    parser.add_argument('-simout','--simout', dest='simout', action='store', required=True, help='Output directory for simulated summed count table files [Required]')
...
def sum_counts_sim(path,df_sim_design,out):
    ##groupby sampleID; this allows us to select the correct count table files to sum together
    df_grouped = df_sim_design.groupby('sampleID').agg(lambda x: x.tolist())
    ...
    for sampleID, filelist in df_grouped['fqName'].iteritems(): ##iterate grouped df using sampleID as index
        ...
        for fqName in filelist:
            ...
            sampleID2=sampleID.replace('split_sim_', '')
            outfile = out +'/' + 'ase_sum_sim_counts_' + sampleID2 + '.csv' ##create outfile for summed matrix
            dfsum.to_csv(outfile)
    for sample in df_sim_design['sampleID']:
        sample=sample.replace('split_sim_', '')
        sample=sample + '_sim'
        Slist.append(sample)
    print(Slist)
    print(len(Slist))
    print(len(df_sim_design['G1']))
    df_sim_design['F1ID']=Slist
    df_drop = df_sim_design.drop(['fqName','sampleID'], axis=1)
#    df_drop = df_sim_design.drop(['sampleID'], axis=1)
    df_unique = df_drop.drop_duplicates()
    df_unique.to_csv(os.path.join(out,"df_ase_sim_samcomp_summed.csv"), index=False)

def sum_counts_data(path,df_design,feature_lengths,out):
    ##groupby sampleID; this allows us to select the correct count table files to sum together
    ...
    row = -1 ##this variable keeps track of which row in the design file (or subset design file) the filename would have been located in; we use this variable to extract the corresponding read length from the design file
    ## Create list of filenames, iterate, put into a df
    for sampleID, filelist in df_grouped['fqName'].iteritems(): ##iterate grouped df using sampleID as index
        ddict = {} 
        dfsum = pd.DataFrame(index=range(0),columns=['FEATURE_ID','BOTH_EXACT', 'BOTH_INEXACT_EQUAL', 'SAM_A_ONLY_EXACT', 'SAM_B_ONLY_EXACT', 'SAM_A_EXACT_SAM_B_INEXACT', 'SAM_B_EXACT_SAM_A_INEXACT', 'SAM_A_ONLY_SINGLE_INEXACT', 'SAM_B_ONLY_SINGLE_INEXACT', 'SAM_A_INEXACT_BETTER', 'SAM_B_INEXACT_BETTER'])
        ...
        for fqName in filelist:
            ...
            ddict[filename]['total x RL'] = ddict[filename]['total_reads_counted']*readLength
            ddict[filename] = ddict[filename].set_index('FEATURE_ID')
            dfsum = dfsum.add(ddict[filename], fill_value =  0)
            ...
            if 'no_etoh' in sampleID:
                sampleID=sampleID.replace('no_etoh','noetoh')
#            G2 = df_design.iloc[row].loc['G2'] ##get G2 from design
    ...
    df_design['ase_compare'] = ' '
    df_drop = df_design.drop(['fqName','readLength', 'fqExtension', 'techRep'], axis=1)
    df_unique = df_drop.drop_duplicates()
    df_unique.to_csv(os.path.join(out,"df_ase_samcomp_summed.csv"), index=False)

def main():
    args = getOptions()

    ## Read in design file as dataframe
    df_sim_design = pd.read_csv(args.simdesign)
    ...
    if args.begin is not None and args.end is not None:
        ...
        df_sim_design = df_sim_design[begin:end]
    ...
    if args.sim == 'True':
        df_sim_design.columns=['G1', 'G2', 'readLength', 'sampleID', 'fqName']
        
        sum_counts_sim(args.simpath,df_sim_design,args.simout)
        ...
        sum_counts_data(args.path,df_design, feature_lengths,args.out)
